...
class AuthToken(models.Model):
    ...
    def set_token(self, expires_in: int = constants.TOKEN_EXPIRES_MINUTES):
        ...
        self.token_key_expires = timezone.now() + datetime.timedelta(
            minutes=expires_in
        )
        self.save()
    ...
    def is_resend_allowed(self):
        if self.last_sent_time and timezone.now() < self.token_key_expires:
            time_difference_from_last_sent_time = (
                timezone.now() - self.last_sent_time
            )
            time_gap = datetime.timedelta(
                minutes=constants.RESEND_EMAIL_MINUTES
            )
...
class ForgotPasswordToken(AuthToken):
    user = models.OneToOneField(
        User, on_delete=models.CASCADE, related_name="forgot_password_token"
    )


class ForgotUsernameToken(AuthToken):
    user = models.OneToOneField(
        User, on_delete=models.CASCADE, related_name="forgot_email_token"
    )
...
class ConfirmMailToken(AuthToken):
    ...
    user = models.OneToOneField(
        ...
        null=True,
        related_name="confirm_mail_token",
    )
...
class Status(models.Model):
    ...
    token = models.ForeignKey(
        ConfirmMailToken, on_delete=models.SET_NULL, blank=True, null=True
    )
