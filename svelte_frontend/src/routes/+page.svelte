...
<div class="flex flex-col p-7 items-center gap-12">
    <h1 class="text-4xl font-black text-center">Welcome, {$page.data.current_user.username}</h1>

    <Form action="?/create_todo" method="post"
          className="flex flex-col items-center justify-center rounded-lg w-full max-w-3xl gap-3">
        Add your todo
        <div
                class="flex flex-col md:flex-row items-center justify-center gap-3 bg-neutral-100 dark:bg-neutral-950 rounded
    ...
    <div class="flex flex-col gap-4 w-full items-center">
        {#each $page.data.todos.items as todo}
            <div class="flex flex-row items-center gap-3 bg-white dark:bg-neutral-950/50 shadow
