...
<script lang="ts">
	...
	function handleModalAfter(res: FormActionResult<Record<string, unknown>>) {
		if (res.type === 'success') {
			closePopup();
		}
	}
...
<Form
	class="flex h-full w-full flex-row items-center gap-3 rounded bg-white p-3 shadow dark:bg-neutral-950"
>
	<BiCircleFill class="w-4 text-blue-500" display />
	...
	<div class="w-full">
		...
		<div class="flex w-full flex-col">
			<div class="flex flex-col justify-between">
				<div class="flex flex-row flex-wrap justify-between gap-1 sm:gap-2">
			...
			<div class="ml-auto flex flex-row flex-wrap justify-between gap-1 text-xs md:gap-2">
				<button type="submit" class={actionButton({ intent: 'danger' })} formaction="?/delete_todo">
					delete
				</button>
				<button
					type="button"
...
{#if is_edit_popup_enabled}
	<div class={modalBase()}>
		...
		>
			...
			<button type="button" class={actionButton({ intent: 'danger' })} onclick={closePopup}>
				cancel
			</button>
		</Form>
	</div>
